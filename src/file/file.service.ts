...
export class FileService {
    ...
    async processFile(body: UUIDDto, userId: ObjectId): Promise<{ url: string }> {
        ...
        try {
            ...
            const img = await sharp(buffer).resize({ width: 1024, withoutEnlargement: true }).toFormat('jpeg', { quality: 80 }).toBuffer();
            const putObjectCommand = new PutObjectCommand({ Bucket: process.env.AWS_FINAL_BUCKET as string, Key: body.uuid.split(".")[0] + ".jpeg", Body: img });
            await this.client.send(putObjectCommand);

            fileData.processed = true;
            fileData.finalUrL = body.uuid.split(".")[0] + ".jpeg";
            await this.conn.db.collection<FileModelDto>("file").updateOne({ _id: fileData._id }, { $set: fileData }).catch(error => { throw error; })
            return { url: body.uuid.split(".")[0] + ".jpeg" };
        } catch (error) {
            throw error;
        }
    }
