...
export class UserService {
    ...
    async create(body: UserCreateDto): Promise<void> {
        if (body.phone.length > 0) {
            this.cloudwatchService.cloudwatchHoneypot({ user: body.username, message: "Honeypot phone - " + body.phone });
        ...
        this.cloudwatchService.cloudwatchRegister({ user: body.username, message: "Register - 200" });
    ...
    async login(body: UserLoginDto): Promise<JWTDto> {
        ...
        if (await bcrypt.compare(body.password, user.password) == true) {
            this.cloudwatchService.cloudwatchLogin({ user: body.username, message: "Login - 200" });
